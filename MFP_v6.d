...
class Model{
	...
	double g,dt_0,dt_min,dt_max,dP_max,dS_max,ssv,dt_down_f,dt_up_f,t_start,t_end,print_interval,abs_convg_SOR,omega_SOR;
	int num_cells,update_mode,solver_type,max_iter_SOR;
	string line_input;	
	string[] parsed_input;
		...
		while (!input_file.eof()) {
			...
			switch (parsed_input[0]) {
				...
				case "dt_max": 						// maximum time step
					...
					break;
				case "dP_max": 		 				// maximum pressure change per time step
					this.dP_max = to!double(parsed_input[1]);
					break;		
				case "dS_max": 		 				// maximum implied saturation change per time step
					this.dS_max = to!double(parsed_input[1]);
					break;					
				...
				case "dt_up_f": 		 			// factor used to grow time step, when dP_max is not exceeded, until dt_max is reached
					...
					break;	
				case "t_start": 		 				// start-of-simulation
					this.t_start = to!double(parsed_input[1]);
					break;		
				case "t_end": 		 				// end-of-simulation
				...
				case "print_interval": 		 				// print output time interval
					...
					break;
				case "update_mode": 		 				// approach for updating total pressure (0 = use PDE solution directly; 1 = implied by phase saturations)
					this.update_mode = to!int(parsed_input[1]);
					break;										
				case "solver_type": 		 				// matrix solver type (0 = successive over-relaxation; 1 = tridiagonal; for 1-D problems only)
					this.solver_type = to!int(parsed_input[1]);
					break;					
				case "abs_convg_SOR": 		 				// pressure-change (per time step) absolute convergence criteria
					this.abs_convg_SOR = to!double(parsed_input[1]);
					break;					
				case "max_iter_SOR": 		 				// maximum number of SOR iterations
					this.max_iter_SOR = to!int(parsed_input[1]);
					break;					
				default: 							// relaxation parameter used in SOR solver (1 < omega < 2)
					assert(parsed_input[0] == "omega_SOR");
					this.omega_SOR = to!double(parsed_input[1]);
					break;
				}		
			}		
		input_file.close();
	...
	void Simulate(Cell[] cell,Connection[] connection,Liquid[] liquid,Source source_w,Source source_n){
	
		// step through simulation ... 
		double time,dt,print_time,dt_preprint,dSw_min,dSw_max,dSn_min,dSn_max;
		...
		double[] Qn;
		string file_name;
		bool dP_pass,dS_pass,print_flag;
		...
		file_name = "state_init.txt";
		WriteState(file_name,cell,liquid,g,dSw,dSn,dt_0);
		file_name = "velocities_init.txt";		
		WriteVelocities(file_name,cell,connection,liquid,g);
		file_name = "fluxes_init.txt";		
		WriteFluxes(file_name,connection,cell,liquid,g);
		...
		do {
			...
			// reconcile time step and print time
			...
			// outer iteration is based on saturation changes and implied pressure changes being less than threshold
			do {
			
				dS_pass = true; 		
				
				do {							// inner iteration: all dP <= dP_max or dt = dt_min
					dP_pass = true;
					Qw = source_w.Q_time(time); 				// return source fluxes at beginning of current time step
					Qn = source_n.Q_time(time);		
					dP = SolveStep(cell,connection,liquid,Qw,Qn,dt,dP_s,dP_s0);		// provisional total fluid pressure change
					// check implied pressure change, but only if implied P & S changes are good (from prior time step)
					if (dS_pass == true){
						for (int i = 0; i < num_cells; ++i){		
							if (abs(dP[i]) > dP_max){
								...
								dP_pass = false;
								dt *= dt_down_f;
								assert(dt>=dt_min,"Inner iteration failure: dt too small at cell " ~to!string(i) ~" where P = " ~to!string(cell[i].P) ~" and dP = " ~to!string(dP[i]));
								break;
								}
							}
						}
				...
				// process and check implied changes in saturation and pressure in response to fluxes
				...
				for (int i = 0; i < num_cells; ++i){
					// check for threshold exceedance violations
					if ((abs(dSw[i])>dS_max) || (abs(dSn[i])>dS_max)){
						dS_pass = false;
						dt *= dt_down_f;
						if (dt < dt_min){
							writeln("Outer iteration failure.");
							writeln("Time = ","\t",time);					
							writeln("Cell no. = ","\t",i);
							writeln("dSw = ","\t",dSw[i]);
							writeln("dSn = ","\t",dSn[i]);	
			...
			if (update_mode == 0){for (int i = 0; i < num_cells; ++i){cell[i].P += dP[i];}}
			else{for (int i = 0; i < num_cells; ++i){cell[i].P = cell[i].Pressure(liquid,g);}}
			...
			if (abs(dSw_min/dt) <= ssv && abs(dSw_max/dt) <= ssv && abs(dSn_min/dt) <= ssv && abs(dSn_max/dt) <= ssv){
				// steady-state flow condition reached
				file_name = "state_" ~ to!string(time) ~ ".txt";
				WriteState(file_name,cell,liquid,g,dSw,dSn,dt);
				file_name = "velocities_" ~ to!string(time) ~ ".txt";				
				WriteVelocities(file_name,cell,connection,liquid,g);
				file_name = "fluxes_" ~ to!string(time) ~ ".txt";				
				WriteFluxes(file_name,connection,cell,liquid,g);					
			...
			if(print_flag == true){dt = dt_preprint;}
	...
	double[] SolveStep(Cell[] cell,Connection[] connection,Liquid[] liquid,double[] Qw,double[] Qn,double dt,double[] dP,double[] dP0){
		...
		double[] a_d,b;
		int iternum = 0;
		bool convg_flag_SOR = false;	
		auto submatrix = BuildMatrix(cell,connection,liquid,Qw,Qn,dt);
		a_d = submatrix[0];
		a = submatrix[1];
		b = submatrix[2];
		if (solver_type==0){
			while (convg_flag_SOR == false) {
				++iternum;
				dP = SOR(a_d,a,b,dP,dP0,cell); 								// iterate on solution
				convg_flag_SOR = CheckConvgSOR(dP,dP0,iternum);                       	// check for convergence
	...
	double[] Tridiagonal(double[] b,double[][] a_od,double[] r,Cell[] cell){
		// tri-diagonal banded matrix solver
		double bet;
		double[] a;
		double[] c;
		double[] u;
	...
	double[] SOR(double[] a_d,double[][] a,double[] b,double[] x,double[] x0,Cell[] cell) {
		...
		for (int i = 0; i < num_cells; ++i) {
			...
			x[i] = (1.0 - omega_SOR) * x0[i] + (omega_SOR/a_d[i]) * (b[i] - sum_0 - sum_1);			
			}
		return x;
		}	
...
class Cell{
	...
	this(double x,double y,double z,double Sw,double Sn,double k,double phi,double vol,double alpha,double n,double Swr,double Snr,double Snr_m,double Ss,double g){
		...
		this.Swr = Swr;
		this.Snr = Snr;
		this.Snr_m = Snr_m; 			// squash mobility of NAPL phase below this saturation (avoids the need to set Snr and introduce extraneous NAPL as an initial condition)
		this.Ss = Ss;
	...
	double kr(int phase){
		...
		Sn_e = Se[1];
		St = Se[2];	
		if (phase == 0){rel_perm = Sw_e^^0.5 * (1.0 - (1.0 - Sw_e^^(1.0/m))^^m)^^2.0;} 		// water
		else{																								// NAPL
			if (Sn > Snr + Snr_m){rel_perm = (St - Sw_e)^^0.5 * ((1.0 - Sw_e^^(1.0/m))^^m - (1.0 - St^^(1.0/m))^^m)^^2.0;}
	...
	Tuple!(double,double,double) SatEff(){
		// effective liquid and total liquid saturation (note: can be slightly > 1 under saturated conditions, depending on Ss value)
		double Sw_e,Sn_e,St;
		Sw_e = min(max((Sw - Swr)/(1.0 - Swr),0.),1.);
		Sn_e = min(max((Sn - Snr)/(1.0 - Snr),0.),1.);
		St = min(max((Sw + Sn - Swr - Snr)/(1.0 - Swr - Snr),0.),1.);
		return tuple(Sw_e,Sn_e,St);
		}

	double Pressure(Liquid[] liquid,double g){
		// (putative) fluid pressure as a function of phase saturation(s)
...
void WriteState(string file_name,Cell[] cell,Liquid[] liquid,double g,double[] dSw,double[] dSn,double dt) {
	// write results to output file
	auto output_file = File(file_name, "w");
	output_file.writeln("cell","\t","x","\t","y","\t","z","\t","Sw","\t","Sn","\t","P","\t","dSw/dt","\t","dSn/dt");
	for (int i = 0; i < cell.length; ++i) {output_file.writeln(i,"\t",cell[i].x,"\t",cell[i].y,"\t",cell[i].z,"\t",cell[i].Sw,"\t",cell[i].Sn,"\t",cell[i].P,"\t",dSw[i]/dt,"\t",dSn[i]/dt);}
	output_file.close();
	}	
...
void WriteFluxes(string file_name,Connection[] connection,Cell[] cell,Liquid[] liquid,double g) {
...
void main(){
	...
	model.Simulate(cell,connection,liquid,source_w,source_n);
